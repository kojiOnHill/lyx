		...
		Context & context, string const & rdelim, string const & rdelimesc)
{
	...
	while (p.good()) {
		...
		if (t.cs() == "textcolor") {
			...
			} else if (preamble.svgnames() && (where = is_known(color, known_svgnames_textcolors))) {
					context.check_layout(os);
			...
			// now check for the other xcolor named colors
					...
					os << "\n\\color inherit\n";
					preamble.registerAutomaticallyLoadedPackage("xcolor");
			// these are the colors we generate for dvipsnames when they clash with svgnames
			} else if (preamble.svgnames() && prefixIs(color, "DVIPS")
				   && (where = is_known(color.substr(5), known_textcolors))) {
					context.check_layout(os);
					os << "\n\\color " << known_coded_textcolors[where - known_textcolors] << "\n";
					parse_text_snippet(p, os, FLAG_ITEM, outer, context);
					context.check_layout(os);
