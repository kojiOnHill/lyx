...
void PrefColors::changeColor(int const &row, bool const &is_dark_mode)
{
	...
	if (setColor(colorsTV_model_.item(row, is_dark_mode), c, color)) {
		// setResetButtonStatus(row, is_dark_mode);
		// emit signal
		changed();
	}
}
...
void PrefColors::setUndoRedoButtonStatuses(bool isUndoing)
{
	...
	if ((!isUndoing && undo_stack_->index() >= 0) ||
		...
		LYXERR0("set undo enabled " << undo_stack_->index()
		        << " : " << undo_stack_->count());
		undoColorPB->setEnabled(true);
	} else {
		LYXERR0("set undo disabled " << undo_stack_->index()
		        << " : " << undo_stack_->count());
	...
	if ((!isUndoing &&
		...
		LYXERR0("set redo enabled " << undo_stack_->index()
		        << " : " << undo_stack_->count());
		redoColorPB->setEnabled(true);
	} else {
		LYXERR0("set redo disabled " << undo_stack_->index()
		        << " : " << undo_stack_->count());
...
void PrefColors::setResetButtonStatus(int const &row)
{
	// light color
	// QColor theme_color = getCurrentThemeColor(lcolors_[row], false);
	...
	if (new_color != theme_color) {
		LYXERR0("light reset color enabled: theme " << theme_color.name() <<
		        ", new " << new_color.name());
		lightColorResetPB->setEnabled(true);
	} else {
		LYXERR0("light reset color disabled: theme " << theme_color.name() <<
		        ", new " << new_color.name());
		lightColorResetPB->setEnabled(false);
	}

	// dark color
	// theme_color = getCurrentThemeColor(lcolors_[row], true);
	theme_color = getCurrentThemeColor(row, true);
	new_color = (QColor)newcolors_[size_t(row)].second;
	if (new_color != theme_color) {
		LYXERR0("dark reset color enabled: theme " << theme_color.name() <<
		        ", new " << new_color.name());
		darkColorResetPB->setEnabled(true);
	} else {
		LYXERR0("dark reset color disabled: theme " << theme_color.name() <<
		        ", new " << new_color.name());
...
QColor PrefColors::getCurrentThemeColor(int const &row, bool const &is_dark_color)
{
	QColor color;
...
// QColor PrefColors::getCurrentThemeColor(QModelIndex selected)
// {
// 	QColor color;
...
// 			color = (QColor)theme_colors_[selected.row()].first;
// 		else if (column == DarkColorColumn)
// 			color = (QColor)theme_colors_[selected.row()].second;
// 	}
// 	return color;
// }
...
SetColor::SetColor(QStandardItem *item, QColor const &new_color,
                   ...
                   ColorNamePairs &cur_color_list, bool const autoapply, PrefColors* color_module,
    ...
    : PrefColors(color_module->form_), QUndoCommand(uc_parent),
      ...
      old_color_(old_color), newcolors_(new_color_list), curcolors_(cur_color_list),
...
void SetColor::setResetButtonStatus()
{
	// int const row = item_.row();
	// int const column = item_.column();

	QModelIndex index = parent_->selection_model_.currentIndex();
	int const row = index.row();
	// int const column = index.column();
	...
	ColorPair colors_to_compare;
	colors_to_compare = newcolors_[row];

	ColorPair const theme_colors_at_row = parent_->getCurrentThemeColors(row);

	LYXERR0("Resetting the reset button at row " << row <<
	        ": theme " << theme_colors_at_row.first.name() <<
	        " compare " << colors_to_compare.first.name());

	// light color
	bool enabled = parent_->lightColorResetPB->isEnabled();
	if (colors_to_compare.first != theme_colors_at_row.first && !enabled) {
		parent_->lightColorResetPB->setDisabled(false);
	} else if (colors_to_compare.first == theme_colors_at_row.first && enabled) {
		parent_->lightColorResetPB->setDisabled(true);
	}

	// dark color
	enabled = parent_->darkColorResetPB->isEnabled();
	if (colors_to_compare.second != theme_colors_at_row.second && !enabled) {
		parent_->darkColorResetPB->setDisabled(false);
	} else if (colors_to_compare.second == theme_colors_at_row.second && enabled) {
		parent_->darkColorResetPB->setDisabled(true);
	}

	if (parent_->undo_stack_->index() < 0)
		parent_->colorResetAllPB->setDisabled(false);
	else
		parent_->colorResetAllPB->setDisabled(true);
