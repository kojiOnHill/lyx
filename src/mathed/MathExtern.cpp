...
void extractIntegrals(MathData & ar, ExternalMath kind)
{
	...
	for (size_t i = 0; i + 1 < ar.size(); ++i) {
		...
		auto p = lyx::make_unique<InsetMathExInt>(buf, from_ascii("intop"));
...
namespace {
	...
	MathData pipeThroughMaxima(docstring const &command, MathData const & ar)
	{
		...
		for (int i = 0; i < 100; ++i) { // at most 100 attempts
			...
			if (ascii_lowercase(out).find("incorrect syntax") == npos)
				break;
			...
			while (is) {
				getline(is, line);
				if (ascii_lowercase(line).find("incorrect syntax") != npos)
					break;
			}
